...
from .models import Chunk, ModuleNotes, Module, Syllabus
...
_NOTES_SYSTEM = """You write study notes strictly from provided source passages.
...
3. If the passages do not cover enough of the module topic to write useful \
notes, reply with exactly one line: the word INSUFFICIENT, then one sentence \
naming what is missing. Write nothing else — no partial notes, no "here is what \
...
def generate_module_notes(
    module: Module, *, namespace: str, cfg: config.RetrievalConfig | None = None
) -> ModuleNotes:
    ...
    body = llm.complete(
        ...
        # The passages are marked cacheable so the milestone-3 quiz call, which
        # reuses this exact context, reads them at a tenth of the input price.
        cached_prefix=f"Source passages:\n\n{_format_passages(chunks)}",
        prompt=(
            f"Module: {module.title}\n\n"
            f"The reader should come away able to:\n{goals}\n\n"
            "Write the study notes for this module."
        ),
...
def run_course(
    ...
    *,
    ...
    with ThreadPoolExecutor(max_workers=config.MAX_PARALLEL_MODULES) as pool:
        notes = list(
            pool.map(
                lambda m: generate_module_notes(m, namespace=namespace, cfg=cfg),
                syllabus.modules,
            )
        )
